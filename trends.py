...
def analyze_tweet_sentiment(tweet):
    ...
    total=0
    word_with_sent= [word for word in word_list if has_sentiment(get_word_sentiment(word))]
    num=len(word_with_sent)
    for word in word_with_sent:
        total+=sentiment_value(get_word_sentiment(word))
    if num==0:
        return make_sentiment(None)        
    else:
        return make_sentiment(total/num)
...
def find_centroid(polygon):
    ...
    "*** YOUR CODE HERE ***"
    def get_latitude(x):
        return latitude(polygon[x])
    def get_longitude(y):
        return longitude(polygon[y])

    vertex_num=list(range(len(polygon)))
    get_x=apply_to_all(get_latitude, vertex_num)
    get_y=apply_to_all(get_longitude, vertex_num)

    def xyxy(i):
        return get_x[i]*get_y[i+1]-get_x[i+1]*get_y[i]
    def x(i):
        return get_x[i]+get_x[i+1]
    def y(i):
        return get_y[i]+get_y[i+1]

    temp_area,temp_x,temp_y,i=0,0,0,0

    while i< len(vertex_num)-1:
        temp_area+= xyxy(i)
        temp_x+=x(i)*xyxy(i)
        temp_y+=y(i)*xyxy(i)
        i=i+1
    ...
    area=temp_area/2
    if area==0:
        return [get_x[0],get_y[0],0.0]
    else:
        x_fin= temp_x/6/area
        y_fin= temp_y/6/area
        return[x_fin,y_fin,abs(area)]
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    get_poly= apply_to_all(find_centroid, polygons)

    area= lambda k: k[2]
    x_area= lambda k: k[0]*k[2]
    y_area= lambda k: k[1]*k[2]

    get_area=apply_to_all(area, get_poly)
    get_x= apply_to_all(x_area, get_poly)
    get_y=apply_to_all(y_area,get_poly)

    sum_area= sum(get_area)
    sum_x=sum(get_x)
    sum_y=sum(get_y)

    final_x=sum_x/sum_area
    final_y=sum_y/sum_area

    return make_position(final_x,final_y)
...
def group_tweets_by_state(tweets):
    ...
    """
    "*** YOUR CODE HERE ***"
    def apply_to_all_on_value(fn,dic):
        dic2={}
        for key in dic:
            dic2[key]=fn(dic[key])
        return dic2

    center_dic= apply_to_all_on_value(find_state_center, us_states) #return values are positions

    def get_state(tweet):
        exact_position= tweet_location(tweet)
        def get_distance(position):
            return geo_distance(exact_position,position)
        all_distance= apply_to_all_on_value(get_distance,center_dic)
        min_distance= min(list(all_distance.values()))
        for key in all_distance:
            if all_distance[key]== min_distance:
                return key

    list_with_name=[]
    for tweet in tweets:
        list_with_name.append([get_state(tweet),tweet])
    return group_by_key(list_with_name)
...
def average_sentiments(tweets_by_state):
    ...
    """
    "*** YOUR CODE HERE ***"
    def average(tweets,total=0,test=0,num=0):
        for tweet in tweets:
    ...
    for key in tweets_by_state:
        if average(tweets_by_state[key])!= 'no sentiment':
            final_dic[key]= average(tweets_by_state[key])
